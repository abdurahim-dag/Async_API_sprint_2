...
async def test_genre_cache(es_init, es_client, random_line, redis_client, make_get_request):
    """Поиск конкретного жанра, с учётом кеша в Redis."""
...
@pytest.mark.parametrize('random_line', [genre_test_settings.data_file_path], indirect=True)
@pytest.mark.asyncio
async def test_genre_list(es_init, es_client, random_line, redis_client, make_get_request):
        """Вывод списка жанров, с учётом кеша в Redis."""
        page_num = 0
        page_size = 5
        params = {
